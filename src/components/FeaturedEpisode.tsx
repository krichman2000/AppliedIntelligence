import Link from "next/link";
import Image from "next/image";
import { Episode } from "@/data/episodes";

type Props = {
  episode: Episode;
};

export function FeaturedEpisode({ episode }: Props) {
  return (
    <div className="border-t border-border pt-10">
      <h2 className="font-serif text-xl font-normal text-charcoal mb-7 flex items-center gap-3">
        Latest Episode
        <span className="inline-block w-8 h-0.5 bg-gold"></span>
      </h2>
      <div className="flex gap-10 flex-wrap">
        {/* Episode Card */}
        <div className="w-80 min-w-[280px] rounded-xl overflow-hidden bg-white shadow-lg flex-shrink-0">
          <div className="relative w-full h-60 overflow-hidden">
            {episode.photo && (
              <Image
                src={episode.photo}
                alt={episode.guest}
                fill
                className="object-cover"
              />
            )}
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4 pt-10">
              <div className="text-white font-semibold text-[15px]">
                {episode.title}
              </div>
              <div className="text-white/80 text-[13px] mt-1">
                with {episode.guest} – {episode.guestTitle}
              </div>
            </div>
          </div>
          <div className="p-4 pt-3">
            {/* Progress bar */}
            <div className="flex items-center gap-2 mb-3">
              <div className="flex-1 h-[3px] bg-border rounded-sm overflow-hidden">
                <div className="w-[35%] h-full bg-blue-600 rounded-sm"></div>
              </div>
            </div>
            {/* Platform icons */}
            <div className="flex gap-2.5 items-center">
              {[
                { color: "#1DB954", icon: "spotify" },
                { color: "#147EFB", icon: "apple" },
                { color: "#FF0000", icon: "youtube" },
              ].map((platform) => (
                <div
                  key={platform.icon}
                  className="w-7 h-7 rounded-full flex items-center justify-center cursor-pointer hover:scale-110 transition-transform"
                  style={{ backgroundColor: platform.color }}
                >
                  <PlatformIcon type={platform.icon} />
                </div>
              ))}
            </div>
          </div>
        </div>

        {/* Episode Details */}
        <div className="flex-1 min-w-[280px] pt-2">
          <h3 className="font-serif text-[28px] font-bold text-charcoal mb-2 leading-tight tracking-tight">
            {episode.title}
          </h3>
          <p className="text-[15px] text-muted mb-5">
            with {episode.guest} — {episode.guestTitle}
          </p>
          <p className="text-[15px] text-body leading-relaxed mb-7">
            {episode.description}
          </p>
          <Link
            href={`/episodes/${episode.id}`}
            className="inline-block px-7 py-3 bg-gold text-white rounded-lg text-[15px] font-semibold hover:bg-gold-dark hover:-translate-y-px transition-all shadow-md shadow-gold/30"
          >
            Play Episode
          </Link>
          <span className="text-[13px] text-muted-light ml-4">
            {episode.duration} &middot; {episode.date}
          </span>
        </div>
      </div>
    </div>
  );
}

function PlatformIcon({ type }: { type: string }) {
  if (type === "spotify") {
    return (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="white">
        <path d="M12 0C5.4 0 0 5.4 0 12s5.4 12 12 12 12-5.4 12-12S18.66 0 12 0zm5.521 17.34c-.24.359-.66.48-1.021.24-2.82-1.74-6.36-2.101-10.561-1.141-.418.122-.779-.179-.899-.539-.12-.421.18-.78.54-.9 4.56-1.021 8.52-.6 11.64 1.32.42.18.479.659.301 1.02zm1.44-3.3c-.301.42-.841.6-1.262.3-3.239-1.98-8.159-2.58-11.939-1.38-.479.12-1.02-.12-1.14-.6-.12-.48.12-1.021.6-1.141C9.6 9.9 15 10.561 18.72 12.84c.361.181.54.78.241 1.2zm.12-3.36C15.24 8.4 8.82 8.16 5.16 9.301c-.6.179-1.2-.181-1.38-.721-.18-.601.18-1.2.72-1.381C8.64 5.801 15.72 6.001 20.1 8.82c.54.3.72 1.02.42 1.56-.299.421-1.02.599-1.439.3z" />
      </svg>
    );
  }
  if (type === "apple") {
    return (
      <svg width="14" height="14" viewBox="0 0 24 24" fill="white">
        <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.81-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z" />
      </svg>
    );
  }
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="white">
      <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
    </svg>
  );
}
